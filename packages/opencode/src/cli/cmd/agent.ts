...
// permission (for example write/edit/apply_patch all gate on `edit`) so we
// configure agents at the permission level to match runtime enforcement.
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        if (perms !== undefined) {
          selected = perms
            ? [...new Set(perms.split(",").map((t) => t.trim()).map(ShellToolID.normalize).filter(Boolean))]
